...
from dataset.JointDepthsDataset import JointDepthsDataset


logger = logging.getLogger(__name__)


class UNRLDepthDataset(JointDepthsDataset):
    ...
    def evaluate(self, cfg, preds, output_dir, *args, **kwargs):
        ...
        pos_gt_src = np.zeros((self.num_joints, 3, len_anno))
        ...
        jnt_error = pos_pred_src - pos_gt_src
        jnt_err = np.linalg.norm(jnt_error[:,:2,:], axis=1)
        depth_err = abs(jnt_error[:,2,:])
        ...
        # scale = np.multiply(bboxsize, np.ones((len(jnt_err), 1)))
        scaled_jnt_err = np.sqrt(np.divide(jnt_err, scale)**2 + (10*depth_err)**2)
        scaled_jnt_err = np.multiply(scaled_jnt_err, jnt_visible)

        jnt_count = np.sum(jnt_visible, axis=1)

        # <-- for what?
        threshold = 0.5
        less_than_threshold = np.multiply((scaled_jnt_err <= threshold),
                                          jnt_visible)
